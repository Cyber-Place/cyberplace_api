...
const resolvers$1 = {
	Query: {
		list: (_,{ jwt }) => {
			...
			return generalRequest(`${URL$1}/address`, 'GET', _, headers)
		},
	},
	Mutation: {
		add: (_, { address, jwt }) => {
			...
			return generalRequest(`${URL$1}/address`, 'POST', address, headers)
		},
        delete: (_, { address, jwt }) => {
			const headers = {
				Authorization : "Bearer" + jwt
			};
			return generalRequest(`${URL$1}/address`, 'DELETE', address, headers)
		},
	}
};
...
//export const url = 'localhost'
const port$1 = '4500';
const entryPoint$1 = 'searchhistory';
...
const resolvers$2 = {
	Query: {
		...
		historyByUsername: (_, {username}) => {
			return generalRequest(`${URL$2}/user/${username}`, 'GET');
		},
		mySearchHistory: async (_, {jwt}) =>{
			...
			const idHistory = await resolvers$2.Query.historyByUsername(_,{username});
			return resolvers$2.Query.historyById(_,idHistory);
		}
	...
	Mutation: {
		createHistory: (_, { history }) => {
			return generalRequest(`${URL$2}`, 'POST', history);
		},
		updateHistory: (_, { id, history }) => {
			return generalRequest(`${URL$2}/${id}`, 'PUT', history);
		},
		deleteHistory: (_, {id}) => {
			return generalRequest(`${URL$2}/${id}`, 'DELETE');
		},
		addItem: (_, {idHistory, item}) => {
			return generalRequest(`${URL$2}/additem/${idHistory}`, 'POST', item);
		},
		removeItem: (_, {idHistory, idItem}) => {
			const params = {
				idItem : idItem
			};
			return generalRequest(`${URL$2}/removeitem/${idHistory}`, 'DELETE', _, _,params);
		},
		removeAllItems: (_, {idHistory}) => {
			return generalRequest(`${URL$2}/removeallitems/${idHistory}`, 'DELETE');
		},
		addItemMyHistory: async (_, {jwt, item}) =>{
			const history = await resolvers$2.Query.mySearchHistory(_,{jwt});
			const idHistory = history.id;
			return resolvers$2.Mutation.addItem(_,{idHistory, item});
		},
		deleteItemMyHistory: async (_, {jwt, idItem}) =>{
			const history = await resolvers$2.Query.mySearchHistory(_,{jwt});
			const idHistory = history.id;
			return resolvers$2.Mutation.removeItem(_,{idHistory, idItem});
		},
		deleteMyHistory: async (_, {jwt}) =>{
			const history = await resolvers$2.Query.mySearchHistory(_,{jwt});
			const idHistory = history.id;
			return resolvers$2.Mutation.removeAllItems(_,{idHistory});
		}
...
const resolvers$3 = {
	...
	Mutation: {
		createShoppingList: (_, {shoppingList}) => {
			return generalRequest(`${URL$3}`, 'POST', shoppingList);
		},
		updateShoppingList: (_, {id,shoppingList}) => {
			return generalRequest(`${URL$3}/${id}`, 'PATCH', shoppingList);
		},
